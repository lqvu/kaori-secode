...
$szFPLogFN = "ksc-ProcessOneRun-Rank.log";

//////////////////// END FOR CUSTOMIZATION ////////////////////

////////////////////////////////// START ////////////////////////
$szExpConfig = "hlf-tv2013";
$szFPRunConfigFN = "/net/dl380g7a/export/ddn11a6/ledduy/trecvid-sin-2013/experiments/hlf-tv2013/runlist/fusion/hlf-tv2013.nsc.bow.harlap6mul.rgbsift.RA.ksc.tvsin13.fusion.cfg";
$nStartConceptID = 0;
$nEndConceptID = 60;
...
$szFPQRelFN = sprintf("%s/%s/%s/nist.%s/%s",
$arConfig['exp_dir'], $szExpName, $arConfig['ann_dir'], $szExpName, $arConfig['eval_qrel']);
...
for($i=$nStartConceptID; $i<$nEndConceptID; $i++)
{
	...
	//	if(!strstr($szFPRunConfigFN, "cu-vireo374"))
	{
		...
		$arTopScoreList = generateRankList($szFPTestVideoListFN, $szExpScoreRunConceptDir, $szFeatureExt, $nMaxScores=2000);
	...
	/*
	 ...
	 {
		// this is for processing CU-VIREO374 scores
		$szFPRankListFN = sprintf("%s/%s.rank",$szExpResultRunDir, $szConceptName);
		loadListFile($arOutput, $szFPRankListFN);
		}
		*/
}
